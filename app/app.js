...
Total explicado: ${n(s.explained)} · Residual sin explicar: ${n(s.residual)} · Estado: ${s.reconciled?'RECONCILIADO':'SIN RECONCILIAR'}`;
      ...
      const tankLines = tanks.map(t => `  ${nn(t.name)}: API ${nn(t.api)} | Temp ${nn(t.temp)}°F | VCF ${nn(t.vcf)} | GSV ${nn(t.gsv)} BBL | FW ${nn(t.fw)} BBL | TCV ${nn(t.tcv)} BBL`).join('\n');
...
Ullage de origen por tanque (volúmenes GSV, FW y TCV SIEMPRE en barriles/BBL; temperaturas SIEMPRE en grados Fahrenheit/°F):
