  ...
  return `
    <div class="sidebar">
      <div style="padding:12px 10px 4px">
        ...
        <div class="sidebar-item ${state.view==='fleet'?'active':''}" data-action="open-fleet">
...
// Analiza todas las operaciones juntas: tendencias de pérdida por buque,
// crudo y terminal, más detección de outliers. La data acumulada como activo.
function fleetMetrics(ops) {
  const rows = [];
  for (const op of ops) {
    const bl  = (op.modules && op.modules['datos-origen'] && op.modules['datos-origen'].bl) || {};
    const arr = (op.modules && op.modules['ullage-arribo'] && op.modules['ullage-arribo'].totals) || {};
    const dor = (op.modules && op.modules['datos-origen']) || {};
    const blG = parseFloat(bl.gsv), arrG = parseFloat(arr.gsv);
    const delta = (!isNaN(blG) && !isNaN(arrG) && blG) ? (arrG - blG) / blG * 100 : null;
    rows.push({
      code: op.code || '—',
      vessel: (op.vessel && op.vessel.name) || '—',
      client: getClient(op) || '—',
      crude: (op.product && (op.product.crudeName || op.product.type)) || '—',
      port: (dor.destination && dor.destination.portName) || op.port || '—',
      date: dor.inspectionDate || op.createdAt || '',
      blG: isNaN(blG) ? null : blG,
      delta,
    });
  }
  return rows;
}
function _fleetGroup(rows, keyFn) {
  const m = {};
  for (const r of rows) { if (r.delta == null) continue; const k = keyFn(r) || '—'; (m[k] = m[k] || []).push(r.delta); }
...
function buildFleetIntelligence(allOps) {
  ...
  const worst = withData.reduce((w, r) => (w == null || r.delta < w.delta) ? r : w, null);
  const totalGsv = withData.reduce((s, r) => s + (r.blG || 0), 0);
      ...
      <div style="display:flex;flex-direction:column;gap:8px">
        ${groups.map(g => `
          <div style="display:flex;align-items:center;gap:10px">
            <div style="width:150px;font-size:12px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${g.k}">${g.k} <span style="color:var(--muted)">(${g.n})</span></div>
            <div style="flex:1;height:16px;background:var(--line2);border-radius:4px;position:relative;overflow:hidden">
              <div style="position:absolute;left:0;top:0;height:100%;width:${Math.abs(g.avg)/gmax*100}%;background:${dColor(g.avg)};border-radius:4px"></div>
            </div>
            <div style="width:80px;text-align:right;font-family:monospace;font-size:12px;color:${dColor(g.avg)};font-weight:600">${fmtP(g.avg)}</div>
          </div>`).join('')}
      </div>
    </div>`;
  };

  const kpi = (val, label, color) => `<div style="flex:1;min-width:130px;text-align:center;background:var(--white);border:1px solid var(--line);border-radius:10px;padding:14px">
    <div style="font-size:22px;font-weight:800;color:${color||'var(--ink)'}">${val}</div>
    <div style="font-size:11px;color:var(--muted);text-transform:uppercase;letter-spacing:.4px;margin-top:2px">${label}</div></div>`;
  ...
  return `
    ...
    <div class="card">
      ...
      ${outliers.length ? `<div style="overflow-x:auto"><table class="data-table" style="min-width:560px">
        <thead><tr><th>Código</th><th>Buque</th><th>Crudo</th><th>Terminal</th><th style="text-align:right">Δ GSV</th></tr></thead>
        <tbody>${outliers.map(r => `<tr>
          <td style="font-weight:600;color:var(--sea)">${r.code}</td>
          <td style="font-size:12px">${r.vessel}</td>
          <td style="font-size:12px">${r.crude}</td>
          <td style="font-size:12px">${r.port}</td>
          <td style="text-align:right;font-weight:700;color:${dColor(r.delta)}">${fmtP(r.delta)}</td>
        </tr>`).join('')}</tbody>
      </table></div>` : '<div style="font-size:13px;color:#2e7d32;padding:8px 0">✓ Ninguna operación supera ±0.5%. Flota dentro de tolerancia.</div>'}
    </div>

    <div class="card">
      <div class="card-title">Detalle por operación (cronológico)</div>
      <div style="overflow-x:auto"><table class="data-table" style="min-width:600px">
        <thead><tr><th>Fecha</th><th>Código</th><th>Buque</th><th>Cliente</th><th style="text-align:right">Δ GSV</th></tr></thead>
        <tbody>${[...withData].sort((a,b)=>String(a.date).localeCompare(String(b.date))).map(r => `<tr>
          <td style="font-size:11px;color:var(--muted)">${r.date ? new Date(r.date).toLocaleDateString('es-CL') : '—'}</td>
          <td style="font-weight:600;color:var(--sea)">${r.code}</td>
          <td style="font-size:12px">${r.vessel}</td>
          <td style="font-size:12px">${r.client}</td>
          <td style="text-align:right;font-weight:600;color:${dColor(r.delta)}">${fmtP(r.delta)}</td>
        </tr>`).join('')}</tbody>
      </table></div>
    </div>`;
}
...
// Sección de Ullage estilo AMSPEC (tanques + auto VCF/GOV/GSV + summary + Excel),
// reutilizable. `ud` = objeto ullage (con .tanks/.summary/.vcfTabla). `sub` = clave
// donde vive (p.ej. 'ullageOrigen', 'ull1'); los handlers usan data-sub para rutear.
function buildAmspecUllageSection(ud, ctx, sub, uid) {
  ud = ud || {};
  const s = ud.summary || {};
  const tanks = (ud.tanks && ud.tanks.length) ? ud.tanks : TANK_NAMES.map(n => ({ name: n }));
  const esc = v => (v == null ? '' : String(v)).replace(/"/g, '&quot;');
  const sNum = (field, dec = 2) => { const v = s[field]; const disp = (v !== '' && v != null && !isNaN(parseFloat(v))) ? parseFloat(v).toFixed(dec) : (v || ''); return `<input class="field-input" type="text" inputmode="decimal" value="${esc(disp)}" placeholder="—" data-action="save-ull-summary" data-ctx="${ctx}" data-sub="${sub}" data-field="${field}" onblur="this.value=this.value&&!isNaN(parseFloat(this.value))?parseFloat(this.value).toFixed(${dec}):this.value">`; };
  const sField = (l, i) => `<div class="field"><label class="field-label">${l}</label>${i}</div>`;
  const TCOLS = [['measured', 'Ullage Corr. (m)', '0.001'], ['ullageInn', 'Ullage Inn. (m)', '0.001'], ['tov', 'T.O.V. (m³)', '0.001'], ['fwInn', 'FW (m)', '0.001'], ['fw', 'FW Vol (m³)', '0.001'], ['gov', 'G.O.V. (BBL)', '0.001'], ['api', 'API@60', '0.1'], ['temp', 'Temp °F', '0.1'], ['vcf', 'VCF', '0.00001'], ['gsv', 'G.S.V. (BBL)', '0.001']];
  const tankRow = (t, i) => `<tr>
    <td style="width:70px"><input class="tbl-input" style="text-align:center;color:var(--amber);font-weight:700" value="${esc(t.name)}" placeholder="TK" data-action="save-ull-origen" data-ctx="${ctx}" data-sub="${sub}" data-idx="${i}" data-field="name"></td>
    ${TCOLS.map(([f, , step]) => { const auto = ['gov', 'vcf', 'gsv'].includes(f); return `<td><input class="tbl-input" type="number" step="${step}" style="min-width:86px${auto ? ';background:rgba(212,165,74,.08)' : ''}" value="${t[f] != null ? t[f] : ''}" data-action="save-ull-origen" data-ctx="${ctx}" data-sub="${sub}" data-idx="${i}" data-field="${f}" placeholder="—"></td>`; }).join('')}
    <td style="width:28px"><button class="btn-icon-sm" data-action="ull-rm-tank" data-ctx="${ctx}" data-sub="${sub}" data-idx="${i}" title="Quitar tanque">✕</button></td>
  </tr>`;
  return `
    <div class="card">
      <div class="card-title">📄 Cargar Ullage desde Excel (plantilla AMSPEC)</div>
      <div style="font-size:11px;color:var(--muted);margin-bottom:8px">Subí el <b>.xlsx</b> del Vessel Ullage Report y lo replico (tanques + summary).</div>
      <input type="file" accept=".xlsx,.xls" style="font-size:12px" onchange="ullageUploadExcel(this, '${ctx}', 'ull-upl-${uid}', '${sub}')">
      <span id="ull-upl-${uid}" style="font-size:11px;margin-left:10px"></span>
    </div>
    <div class="card">
      <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px">
        ...
        <div style="display:flex;align-items:center;gap:8px">
          ...
          <select class="field-select" style="width:auto" data-action="ull-set-tabla" data-ctx="${ctx}" data-sub="${sub}">
            ${['6A', '6B', '6C', '6D'].map(tb => `<option value="${tb}" ${(ud.vcfTabla || '6A') === tb ? 'selected' : ''}>${tb}</option>`).join('')}
          </select>
          <button class="btn btn-primary btn-sm" data-action="ull-recalc" data-ctx="${ctx}" data-sub="${sub}">🔄 Recalcular</button>
        </div>
      </div>
      <div style="font-size:11px;color:var(--muted);margin:6px 0 10px">Las columnas <b>VCF</b>, <b>G.O.V.</b> y <b>G.S.V.</b> (dorado) se calculan por API MPMS 11.1: G.O.V. = (T.O.V.−FW) × 6.289812 · G.S.V. = G.O.V. × VCF.</div>
      <div style="overflow-x:auto"><table class="data-table" style="min-width:1120px;font-size:12px">
        <thead><tr><th>Tanque</th>${TCOLS.map(([, l]) => `<th>${l}</th>`).join('')}<th></th></tr></thead>
        <tbody>${tanks.map((t, i) => tankRow(t, i)).join('')}</tbody>
      </table></div>
      <button class="btn btn-secondary btn-sm" data-action="ull-add-tank" data-ctx="${ctx}" data-sub="${sub}" style="margin-top:12px">＋ Agregar tanque</button>
    </div>
    <div class="card">
...
function buildDatosOrigen(d, ctx) {
  ...
  return `
    ...
    <div class="card">
      <div class="card-title">Ullage de Origen${multiP ? ` <span style="font-size:12px;font-weight:400;color:var(--amber)">· ${prodLabel(products[activeIdx])}</span>` : ''}</div>
      <div style="font-size:12px;color:var(--muted)">Vessel Ullage Report de origen — misma estructura que el Ullage al Arribo (auto-cálculo y carga de Excel).</div>
    </div>
    ${buildAmspecUllageSection(d[ullSub], ctx, ullSub, 'orig-' + activeIdx)}
    <div class="card">
      <label class="field-label">Notas ullage de origen</label>
      <textarea class="field-input" style="height:60px" placeholder="Notas ullage de origen…"
        data-action="save-nested" data-ctx="${ctx}" data-obj="${ullSub}" data-field="notes">${(d[ullSub]&&d[ullSub].notes)||''}</textarea>
    </div>
...
async function ullageUploadExcel(input, ctxStr, msgId, sub) {
  const file = input.files && input.files[0];
  const msgEl = msgId ? document.getElementById(msgId) : null;
  if (!file) return;
  ...
  try {
    ...
    const ref = getModuleRef(decodeCtx(ctxStr));
    if (!ref) { input.value = ''; return; }
    const dataObj = sub ? (ref.data[sub] || (ref.data[sub] = {})) : ref.data;
    dataObj.header = parsed.header;
    dataObj.tanks = parsed.tanks;
    dataObj.summary = parsed.summary;
    _syncUllageTotals(dataObj);
    ref.save();
...
const KM_BLOCKS = [
  { id:'OP', title:'Acuerdos Clave de la Operación', questions:[
    { id:'k_producto',   text:'Producto / grado a descargar', type:'text' },
    { id:'k_cantidad',   text:'Cantidad a descargar (BBL / MT)', type:'text' },
    { id:'k_stop',       text:'Quién para la operación (responsable de dar la orden de parada)', type:'text' },
    { id:'k_rate_nave',  text:'Rate ofrecido por la nave (BBL/h)', type:'text' },
    { id:'k_rate_term',  text:'Rate que el terminal puede recibir (BBL/h)', type:'text' },
    { id:'k_tanques',    text:'Tanques a utilizar para la descarga', type:'textarea' },
...
function handleClick(e) {
  ...
  if (a === 'go-home') { state.view='home'; state.currentOpId=null; render(); }
  ...
  else if (a === 'ull-recalc') { const _c=decodeCtx(el.dataset.ctx); const _r=getModuleRef(_c); if(_r){ const _s=el.dataset.sub; computeUllageDerived(_s?(_r.data[_s]||{}):_r.data); _r.save(); renderKeepScroll(); } }
    ...
    // Temperaturas — origen (B/L, shore, ship) y arribo (por tanque + promedio).
    const shore = origen.shore || {}, shipO = origen.shipFigureOrigin || {};
    const arrTanks = arribo.tanks || [];
    const arrTempNums = arrTanks.map(t => parseFloat(t.temp)).filter(v => !isNaN(v));
    const arrAvgTemp = arrTempNums.length ? (arrTempNums.reduce((a,b)=>a+b,0)/arrTempNums.length).toFixed(2) : null;
    const arrTankTemps = arrTanks.map((t,i)=>`${t.name||('TK '+(i+1))}: ${(t.temp!==''&&t.temp!=null)?t.temp+'°F':'s/d'}`).join(' · ') || 's/d';
    // Condiciones ambientales del Key Meeting (respuestas de texto libre, si existen).
    const km = op.modules?.['key-meeting'] || {};
    const kmAns = km.answers || {};
    const ambKeys = Object.keys(kmAns).filter(k => /amb|mar|sea|clim|viento|wind|oleaje|weather|temp/i.test(k + ' ' + (kmAns[k]||'')));
    const kmAmb = ambKeys.length ? ambKeys.map(k => kmAns[k]).filter(Boolean).join(' | ') : '';
    const tempBlock = `\n\nTEMPERATURAS — TODAS EN GRADOS FAHRENHEIT (°F), NO Celsius:\n- Origen — B/L: ${origen.bl?.temp||'s/d'} · Tierra/Shore: ${shore.temp||'s/d'} · Figura buque: ${shipO.temp||'s/d'}\n- Arribo — promedio de tanques: ${arrAvgTemp!==null?arrAvgTemp+'°F':'s/d'}\n- Arribo por tanque: ${arrTankTemps}${kmAmb?`\n- Condiciones ambientales (Key Meeting): ${kmAmb}`:''}\nIMPORTANTE: interpretá estos valores como °F (ej. 74 °F ≈ 23 °C). Usalas para el análisis de contracción/expansión térmica (Cap. 7 / 11.1). Si alguna figura como "s/d" es porque no fue ingresada; señalalo pero NO afirmes que faltan todas.`;
...
Total explicado: ${n(s.explained)} · Residual sin explicar: ${n(s.residual)} · Estado: ${s.reconciled?'RECONCILIADO':'SIN RECONCILIAR'}`;
      ...
      const tabla = ull.vcfTabla || '6A';
      // VEF viaje por viaje con ratio, calificación y rechazo (ya calculado por el motor)
      const voyLines = voyages.map((v, i) => {
        const r = vs && vs.rows ? vs.rows[i] : null;
        const ratio = r && r.ratio != null ? r.ratio.toFixed(5) : '—';
        const estado = r ? (r.isRejected ? 'RECHAZADO' : (r.qualifying ? 'califica' : 'no califica')) : '—';
        const gerr = r && r.grossErr ? ' · GROSS ERROR >2%' : '';
        return `  ${i + 1}. ${nn(v.date)} | Viaje ${nn(v.voyageNum)} | ${nn(v.terminal)} | ${nn(v.product)} | TCV nave ${nn(v.vesselTCV)} | OBQ/ROB ${nn(v.obq)} | TCV shore ${nn(v.shoreTCV)} | ratio V/S ${ratio} | ${estado}${gerr} | cat ${nn(v.category)}`;
      }).join('\n');
      // Ullage por tanque con VCF recalculado (11.1) vs reportado → Δ para verificación
      const tankLines = tanks.map(t => {
        const api = parseFloat(t.api), temp = parseFloat(t.temp);
        let exp = null; if (!isNaN(api) && !isNaN(temp)) { try { exp = vcfCalc(api, temp, tabla); } catch (e) {} }
        const rep = parseFloat(t.vcf);
        const dv = (exp != null && !isNaN(rep)) ? (rep - exp).toFixed(5) : 'N/D';
        return `  ${nn(t.name)}: API ${nn(t.api)} | Temp ${nn(t.temp)}°F | VCF reportado ${nn(t.vcf)} vs VCF esperado 11.1 (Tabla ${tabla}) ${exp != null ? exp.toFixed(5) : 'N/D'} (Δ ${dv}) | GSV ${nn(t.gsv)} BBL | FW ${nn(t.fw)} BBL`;
      }).join('\n');
      const originData = `DATOS DE ORIGEN
Buque: ${op.vessel?.name || '—'} | Producto: ${op.product?.crudeName || op.product?.type || '—'}
B/L N° ${nn(bl.blNumber||modData.blNumber)} | Fecha de carga: ${nn(bl.blDate||modData.blDate)} | Puerto/Zona de carga: ${nn(bl.loadPort||modData.loadPort)} | Terminal: ${nn(bl.loadTerminal||modData.loadTerminal)}
Cantidades B/L: GSV(gross)=${nn(bl.gsv)} BBL | NSV(net)=${nn(bl.nsv)} BBL | API@60=${nn(bl.api)} | BS&W=${nn(bl.bsw)}%
VEF de origen calculado por el motor: ${vs ? vs.vef.toFixed(5) : 'N/D'} · viajes calificantes: ${vs ? vs.qualCount : 0} de ${voyages.length}${vs && vs.qualCount < 5 ? ' (¡< 5 → VEF forzado a 1.0!)' : ''}
Historial de viajes del VEF (ratio esperado ≈ TCV descargado / TCV shore, banda ±0.3%):
${voyLines || '  (sin viajes cargados)'}
Ullage de origen por tanque — volúmenes en BBL, temperaturas en °F. Se incluye el VCF esperado por API MPMS 11.1 para que verifiques cada uno:
${tankLines || '  (sin tanques cargados)'}
Observaciones generales del operador: ${nn(modData.notes || ull.notes)}`;
      prompt = `Eres un QPIC / Inspector Senior de Loss Control con 25+ años en custody transfer de crudo (API MPMS Cap. 7, 9.1, 10, 11.1, 12, 17.9). NO des una respuesta genérica ni de plantilla: analiza EXCLUSIVAMENTE los números de ESTOS datos, citándolos. Si un dato falta, dilo; no inventes.\n\n${opCtx}\n\n${originData}\n\nEjecutá OBLIGATORIAMENTE estos chequeos, cada uno con las cifras concretas:\n\n1. VEF — no lo omitas. Comenta el valor (${vs ? vs.vef.toFixed(5) : 'N/D'}) y viaje por viaje: nombrá los que están RECHAZADOS o con GROSS ERROR y por qué; señalá ratios fuera de banda ±0.3%; detectá viajes que parezcan ALIJE/STS (por el terminal, cargo o comentario) que no deberían calificar; si hay <5 calificantes, advertí que el VEF=1 no corrige nada. ¿El VEF es confiable?\n2. VCF POR TANQUE — para cada tanque compará el VCF reportado con el VCF esperado 11.1 que te di (columna Δ). Si algún Δ es ≠ 0 significativo, marcá ese tanque como VCF INCORRECTO (posible error de digitación de API/Temp o de tabla). Listá los tanques con problema.\n3. COHERENCIA TEMPERATURA vs ZONA DE CARGA — dado el PUERTO y la FECHA de carga, ¿la temperatura de carga (en °F) es coherente con la temperatura ambiente/agua típica de esa zona y época? Si el crudo se cargó mucho más caliente que el ambiente (calentado) o incoherente, decilo y explicá el impacto.\n4. SESGOS SISTEMÁTICOS — nombralos y dá su DIRECCIÓN (sobre/subestimación de la figura de origen): estratificación térmica en crudo recién cargado, medición superficial, API/densidad, etc.\n5. DESVÍOS ESPERADOS AL ARRIBO A CHILE — estimá el sentido y magnitud de la diferencia esperada al arribo al próximo puerto: contracción por enfriamiento (agua fría del Pacífico sur) usando la ΔT esperada, más el efecto de los errores sistemáticos detectados. Distinguí qué parte sería física (no pérdida) de una posible pérdida real. Dá un rango orientativo.\n6. DICTAMEN Y ACCIONES concretas (re-medir tanques X, exigir medición por zonas, LOP, nota al cliente).\n\nSé específico y numérico. Prohibido responder en genérico.`;
    } else {
...
function handleChange(e) {
  ...
  else if (a === 'save-ull-origen') { const _s=el.dataset.sub || 'ullageOrigen'; saveUllTank(el.dataset.ctx, _s, parseInt(el.dataset.idx), el.dataset.field, el.value); const _c=decodeCtx(el.dataset.ctx); const _r=getModuleRef(_c); if(_r){ computeUllageDerived(_r.data[_s]||{}); _r.save(); renderKeepScroll(); } }
  ...
  else if (a === 'save-ull-summary') { const _c=decodeCtx(el.dataset.ctx); const _r=getModuleRef(_c); if(_r){ const _s=el.dataset.sub; const _t=_s?(_r.data[_s]||(_r.data[_s]={})):_r.data; if(!_t.summary)_t.summary={}; _t.summary[el.dataset.field]=el.value; if(el.dataset.field==='vef'){ computeUllageDerived(_t); renderKeepScroll(); } else { _syncUllageTotals(_t); } _r.save(); } }
  else if (a === 'ull-set-tabla') { const _c=decodeCtx(el.dataset.ctx); const _r=getModuleRef(_c); if(_r){ const _s=el.dataset.sub; const _t=_s?(_r.data[_s]||(_r.data[_s]={})):_r.data; _t.vcfTabla=el.value; computeUllageDerived(_t); _r.save(); renderKeepScroll(); } }
...
function handleInput(e) {
  ...
  else if (a === 'save-ull-summary') { const _c=decodeCtx(el.dataset.ctx); const _r=getModuleRef(_c); if(_r){ const _s=el.dataset.sub; const _t=_s?(_r.data[_s]||(_r.data[_s]={})):_r.data; if(!_t.summary)_t.summary={}; _t.summary[el.dataset.field]=el.value; _syncUllageTotals(_t); _r.save(); } }
  else if (a === 'save-slop') saveSlop(el.dataset.ctx, el.dataset.phase, parseInt(el.dataset.idx), el.dataset.field, el.value);
  else if (a === 'save-nested') saveNested(el.dataset.ctx, el.dataset.obj, el.dataset.field, el.value);
